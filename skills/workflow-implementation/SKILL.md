...
`.ai/tasks/...`, or `.ai/project.md` at `profile: lite`) are relative
to the project root, not to this skill file — write the full
...
2. The task's detail — Context + Implementation (Files touched, Steps,
   and Pseudocode at `profile: full`). Where this lives depends on
   `.ai/info.md`'s `profile:` (see
   [.ai/workflow/workflow.md §14](.ai/workflow/workflow.md#14-profiles)):
   its own file (`.ai/tasks/p{NN}-t{NN}-{name}.md`) at `full`; an
   inline block under its row in the owning `.ai/phases/p{NN}-{name}.md`
   Tasks table at `medium`; an inline block under its row in
   `.ai/project.md`'s Roadmap table at `lite`.
...
3. Check the task's current Status in its owning Tasks table
   (the phase file's at `full`/`medium`, `.ai/project.md`'s Roadmap
   table at `lite` — the task's own detail never tracks its own
   status; that table is the only place it lives). It
...
4. Follow the task's Files-to-modify/Files-to-create (or merged Files
   touched, at `medium`/`lite`) and Steps in order. If Pseudocode is
   present (`full` only — never written at `medium`/`lite`, §14), treat
   it as guidance for the approach, not a literal script — adapt it to
   what you actually find in the codebase. Minor mismatches (a function
   living in a different file than expected, or an implementation
   detail that differs from the pseudocode's specifics) — adjust and
   continue, no deviation needed; see [.ai/workflow/workflow.md §6](.ai/workflow/workflow.md#6-deviations).
...
1. Set the task's Status to `validating` (or `blocked` if stuck) in
   its owning Tasks table (the phase file's at `full`/`medium`,
   `.ai/project.md`'s Roadmap table at `lite`), and update
   `.ai/info.md` to match. These two are the only places task status
   lives — nothing else needs updating.
2. Commit (see [.ai/workflow/workflow.md §13](.ai/workflow/workflow.md#13-commit-discipline)).
   At `full`/`medium`: stop for `task-validation` — see `.ai/info.md`
   ...
   At `lite`, `task-validation` isn't a separate gate (§14) — proceed
   directly into `workflow-validation`'s checks, which continue
   straight through `workflow-review` into the single combined
   `task-completion` stop; only that final stop pauses for approval.
...
Modified project files; an updated row in the owning Tasks table (the
phase file's at `full`/`medium`, `.ai/project.md`'s Roadmap table at
`lite`); `.ai/info.md` updated; a new ADR and
